<template>
    ...
    <div class="bg-white relative rounded-lg mt-10">
        ...
        <div class="grid sm:grid-cols-[100%] md:grid-cols-[30%_70%]" v-if="!isFetchTypeCustomer && Object.keys(typeCustomer).length != 0">
            <div class="border-r">
                <div
...
<script setup lang="ts">
    ...
    const typeCustomer = ref<TypeCustomer>({});
    ...
    const fetchUpdateCustomer = async () => {
        try {
            isLoader.value = true;
            const res = await apiPut(`type-customers/${route.params.id}`, typeCustomer.value);
